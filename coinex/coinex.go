...
const coinexFuturesTakerFee = 0.05

func FetchFutures() []models.FutureResult {
	client := &http.Client{Timeout: 10 * time.Second}
	tickers, err := fetchCoinExFuturesTickers(client)
	if err != nil {
		return nil
	}
	...
	for _, t := range tickers {
		...
		// 🔴 Множитель для цен CoinEx мы также берем строго из нормализатора (1.0 для BTC/HIGH)
		...
		results = append(results, models.FutureResult{
			...
			Symbol:      t.Symbol,
			BaseCoin:    baseName,
			Multiplier:  mult, // 🔴 Только чистый mult!
			Bid:         t.BidPrice,
			Ask:         t.AskPrice,
			BidSize:     t.BidSize,
			AskSize:     t.AskSize,
			...
			Deviation:   dev,
			TakerFee:    coinexFuturesTakerFee,
		})
	}
	return results
}
